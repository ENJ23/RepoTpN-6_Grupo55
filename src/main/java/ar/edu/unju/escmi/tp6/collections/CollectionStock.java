...
public class CollectionStock {
	...
	public static void stockAhora30() {
		
		for (Producto prod : CollectionProducto.productos) {
			if (prod.getOrigenFabricacion().equals("Argentina")) {
				if (stocks != null) {
					for (Stock sto : stocks) {
						if (sto.getProducto() == prod) {
							System.out.println("Disponibles: " + sto.getCantidad() + " Cod: " + sto.getProducto().getCodigo() + "---" + sto.getProducto().getDescripcion());
						}
					}
				}
			}
